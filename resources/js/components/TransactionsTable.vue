<template>
  <v-client-table
    ...
    :options="{
      ...
      sortable: ['date', 'account', 'category', 'payee', 'amount', 'cleared'],
      customSorting: {
  ...
  >
    ...
    <template #actions="{ row }" v-if="showActions">
      <button
        ...
        class="button button--warning w-8 text-gray-100 p-1"
        @click="$root.$emit('editTransaction', row.id)"
      ...
      <button
        ...
        @click="$root.$emit('deleteTransaction', row.id)"
...
export default {
  ...
  data() {
    return {
      defaultColumns: ['flair', 'date', 'account', 'category', 'payee', 'amount', 'cleared'],
    }
  },
  computed: {
    tableHeaders() {
