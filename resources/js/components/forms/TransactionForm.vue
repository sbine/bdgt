	...
	<modal :value="creating || editing || deleting" @input="reset">
        <form ref="form" :method="formMethod" :action="formAction" @submit.prevent="save">
            <div class="sm:w-5/6 p-8">
                ...
                <template v-else>
                    <h4 class="text-2xl mb-10">{{ editing ? $t('labels.transactions.modals.edit.title') : $t('labels.transactions.modals.create.title') }}</h4>
                    ...
                    <div class="form-row">
                        ...
                        <div class="form-row__input input-addon--start">
                            ...
                            <input v-model="transaction.amount" type="number" class="input-text pl-10" name="amount" step="0.01" min="0" max="1000000" required>
                        </div>
                    </div>
            ...
            <div v-if="action" class="flex justify-end bg-gray-100 border-t px-8 xl:px-10 py-6">
                <button type="button" class="link mr-6" @click="reset">{{ $t(`labels.transactions.modals.${action}.close_button`) }}</button>
                <button type="submit" class="button button--primary">{{ $t(`labels.transactions.modals.${action}.save_button`) }}</button>
            </div>
        </form>
	</modal>
</template>
        ...
        transactionAmount: {
            get() {
                return this.transaction
                    ? (this.transaction.inflow ? this.transaction.amount : (-1 * this.transaction.amount))
                    : null
            },
            set(value) {
                this.transaction.amount = (this.transaction.inflow
                    ? value
                    : (-1 * value))
            }
        }
